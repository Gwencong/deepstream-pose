...
def dispaly_frame_pose(frame_meta, batch_meta, boxes, confs, kpts, step=3):
    ...
    add_pose_display_meta(frame_meta, batch_meta, kpts, draw_kpts=False, draw_limbs=True)
...
def add_box_display_meta(frame_meta, batch_meta, boxes, confs, draw_boxes=True, draw_confs=True):
    ...
    display_metas = [pyds.nvds_acquire_display_meta_from_pool(batch_meta) for i in range(need_num)]
    display_meta = None
    for i, (box,conf) in enumerate(zip(boxes,confs)):
        ...
        if draw_confs:
            ...
            conf_text_params.display_text = "person:{:.2f}".format(conf.squeeze())
